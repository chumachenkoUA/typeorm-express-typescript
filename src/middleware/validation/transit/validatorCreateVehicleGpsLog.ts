...
export const validatorCreateVehicleGpsLog = async (req: Request, _res: Response, next: NextFunction) => {
  ...
  if (errors.length > 0) {
    return next(
      new CustomError(400, 'Validation', 'Помилка валідації створення GPS-запису транспорту.', null, null, errors),
    );
  }
