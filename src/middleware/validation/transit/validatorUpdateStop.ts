...
export const validatorUpdateStop = async (req: Request, _res: Response, next: NextFunction) => {
  ...
  if (name !== undefined && validator.isEmpty(name ?? '')) {
    errors.push({ name: 'Назва зупинки не може бути порожньою.' });
  }
  ...
  if (errors.length > 0) {
    return next(new CustomError(400, 'Validation', 'Помилка валідації оновлення зупинки.', null, null, errors));
  }
